...
{
    cr_assert_eq(static_cast<int>(Undefined), -1,
        "Undefined should be -1");
    cr_assert_eq(static_cast<int>(False), 0,
        "False should be 0");
    cr_assert_eq(static_cast<int>(True), 1,
        "True should be 1");
}
...
Test(tristate, enum_completeness)
{
    ...
    for (auto v : values) {
        switch (v) {
            case Undefined: count++; break;
            case False: count++; break;
            case True: count++; break;
        }
    }
...
namespace {
    ...
    struct TestableComponent : AComponent {
        ...
        Tristate readLinked(std::size_t pin)
        {
    ...
    struct TestFactory : IComponentFactory {
        std::unique_ptr<IComponent> createComponent(const std::string &) const override
        {
...
Test(parser, links_to_unknown_component_throw)
{
    ...
    cr_assert(thrown, "Expected ParseError when link references unknown component");
}

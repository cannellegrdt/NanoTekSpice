...
void Circuit::addComponent(const std::string &name, std::unique_ptr<IComponent> component) {
  ...
  if (!_components.emplace(name, std::move(component)).second) {
...
IComponent &Circuit::getComponent(const std::string &name) {
  std::map<std::string, std::unique_ptr<IComponent>>::iterator it = _components.find(name);
...
const IComponent &Circuit::getComponent(const std::string &name) const {
  std::map<std::string, std::unique_ptr<IComponent>>::const_iterator it = _components.find(name);
  if (it == _components.end()) {
    throw ParseError("Unknown component: '" + name + "'");
  }
  return *it->second;
...
void Circuit::setInputValue(const std::string &name, const std::string &value) {
  ...
  bool isInput = std::find(_inputNames.begin(), _inputNames.end(), name) != _inputNames.end();
  bool isClock = std::find(_clockNames.begin(), _clockNames.end(), name) != _clockNames.end();
  if (!isInput && !isClock) {
  ...
  AComponent *acomp = dynamic_cast<AComponent *>(&(*it->second));
  if (acomp) {
    acomp->setPin(1, val);
  }
}
...
std::size_t Circuit::tick() const noexcept { return _tick; }
